	...
	for (p = (char *)fmt; (c = *p); p++)
	{
		...
		switch (c = *++p)
		{
			...
			case 'b':
				...
				goto cont_u;
			case 'p':
				uval = (unsigned int)(unsigned long)va_arg(ap, void *);
				vlen = printk_mknumstr(vstr,&uval,FALSE,16);
				goto cont_u;
			case 'u':
				uval = (unsigned int)va_arg(ap, unsigned int);
...
int local_puts(const char* s, int *count, PRINTK_INFO *info)
{
    while (s != 0 && *s != '\0') {
        printk_putc(*s++, count, info);
    }
    return 0;
}
/********************************************************************/
...
// {
	...
	// /*
	 ...
	 // */
	// va_start(ap, fmt);
	// rvalue = sprintf(ibuf, fmt, ap);
	// local_puts(ibuf, &count, &info);
	// /*
        ...
        case 'l':
            ...
            case 'l':
                ...
                case 'i':
					...
					sprintf(ibuf,"%lli",llival); 
                    local_puts(ibuf, &count, &info);
                    break;

                case 'u':
                    lluival = va_arg(ap, unsigned long long int);
                    sprintf(ibuf,"%llu",lluival); 
                    local_puts(ibuf, &count, &info);
                    break;
                case 'x':
                case 'X':
            ...
            case 'i':
                ...
                sprintf(ibuf,"%li",lival); 
                local_puts(ibuf, &count, &info);
                break;

            case 'u':
                luival = va_arg(ap, unsigned long int);
                sprintf(ibuf,"%lu",luival); 
                local_puts(ibuf, &count, &info);
                break;
            case 'x':
            case 'X':
