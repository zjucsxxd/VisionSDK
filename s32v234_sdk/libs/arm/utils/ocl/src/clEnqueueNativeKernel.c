  ...
  for (i = 0; i < num_mem_objects; i++)
    {
      ...
      //CM
      void* pOalMem = mem_list[i]->device_ptrs[command_queue->device->dev_id].mem_ptr;
      buf = OAL_MemoryReturnAddress(pOalMem, ACCESS_PHY);
      OAL_MemoryFlushAndInvalidate(pOalMem, OAL_MemoryGetBytesRequested(pOalMem));
      ...
      unsigned long offset = (uintptr_t) loc - (uintptr_t) args;
