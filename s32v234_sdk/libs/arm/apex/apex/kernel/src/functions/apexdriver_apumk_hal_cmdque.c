...
void
apexdriver_apumk_hal_dmem_init(int apexID, int apuid, const int que_mode)
{
   int64_t base_addr    = 0;
   ...
   {
      base_addr   = (int64_t)ReturnVirtualHwPtr(apexID, HW_REG_SMEM);
   }
                        ...
                        int32_t* list)
{
   ...
   base_addr   = 0;
#else
   base_addr = (int64_t)ReturnVirtualHwPtr(apexID, HW_REG_SMEM) + apexdriver_cmem_if_hal_query_apu_dmem_start(apexID, apu_id);
#endif
                       ...
                       const int32_t* param_list)
{
   ...
   apexdriver_apumk_hal_push_cmd(list, kernel_id, num_params, param_list);
