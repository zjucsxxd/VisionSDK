...
int bitonicSort_AMD__3 (  int nxt,
...
extern int  bitonicSort_AMD__3__md[];

// SET ARGUMENT FUNCTION DECL
void bitonicSort_AMD__3__set(void chess_storage(DMb)* ctx, cl_argument* args);


// KERNEL METADATA
CL_KRNL_MD bitonicSort_AMD_md = 
{
   0,0,0,0,                   // required wg size
   4,        // number of arguments
   3,                         // kernel mode
   1,2,2,2,0,0,0,0,           // arg addr space type
   0,0,0,0,0,0,0,0,           // arg access type
   bitonicSort_AMD__3__md,
   (int32_t)(&bitonicSort_AMD__3),
   (int32_t)(&bitonicSort_AMD__3__set),
...
void bitonicSort_AMD__3__set(void chess_storage(DMb)* ctx, cl_argument* args) {
    ...
    //
    p = kernel_add_int_arg(p, args[1].value);

    //
    p = kernel_add_int_arg(p, args[2].value);

    //
    p = kernel_add_int_arg(p, args[3].value);
